...
/**
 * Win-probability + betting line for a pre-game / live matchup. The
 * win-probability split bar (away left, home right) mirrors the team-stats
 * bar; the moneyline / spread / total rows show only what the provider
 * priced, attributed to the sportsbook. Soccer usually has no line, so this
 * whole section is simply absent there.
 */
function OddsSection({ odds, game }: { odds: GameOdds; game: Game }) {
  const awayLabel = game.away.abbreviation ?? abbrev(game.away.name);
  const homeLabel = game.home.abbreviation ?? abbrev(game.home.name);
